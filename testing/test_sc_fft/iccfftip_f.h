...
static void iccfftip_f()
{
   printf("********\nTEST ccfftip inverse\n");
   {
      ...
      for(i=0; i<4; i++){
        printf("vsip_fft_f *fft = vsip_ccfftip_create_f(np,1.0,VSIP_FFT_INV,0,0)\n");
        printf("np = %4d\n",np[i]);
        ...
        { /* do fft */
          {
          vsip_fft_f *fft = vsip_ccfftip_create_f(np[i],1.0,VSIP_FFT_INV,0,0);
