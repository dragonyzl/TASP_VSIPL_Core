...
**********************************************************************/
/* $Id: rcfftop.t,v 1.1 2000/04/14 12:44:14 judd Exp judd $ */

static void rcfftop_d()
{
   printf("********\nTEST rcfftop\n");
   {
      ...
      for(i=0; i<4; i++){
        printf("vsip_fft_d *fft = vsip_rcfftop_create_d(np,1.0/np,0,0)\n");
        printf("np = %4d\n",np[i]);
        ...
        vsip_cvputlength_d(y,np[i]/2+1);
        vsip_vputlength_d(ramp,np[i]);
        { /* do fft */
          {
          vsip_fft_d *fft = vsip_rcfftop_create_d(np[i],1.0/np[i],0,0);
          printf("x = ");VU_vprintm_d("8.6",ramp);
          vsip_rcfftop_d(fft,ramp,y);
          printf("vsip_rcfftop_d(fft,x,y)\n");
          printf("y = ");VU_cvprintm_d("8.6",y);
          vsip_fft_destroy_d(fft); 
          }
        }
        vsip_cvputlength_d(chk,np[i]/2+1); vsip_vputlength_d(chk_r,np[i]/2+1);
        printf("expected\n");VU_cvprintm_d("7.4",x);
        vsip_cvsub_d(y,x,chk); vsip_cvmag_d(chk,chk_r);
