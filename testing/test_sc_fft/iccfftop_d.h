...
**********************************************************************/
/* $Id: iccfftop.t,v 1.1 2000/04/14 12:44:14 judd Exp judd $ */

static void iccfftop_d()
{  
   printf("********\nTEST ccfftop inverse\n");
   {
      ...
      vsip_cvview_d  *cramp = vsip_cvcreate_d(MAX_POINTS,0);
      ...
      for(i=0; i<4; i++){
        printf("vsip_fft_d *fft = vsip_ccfftop_create_d(np,1.0,VSIP_FFT_INV,0,0)\n");
        printf("np = %4d\n",np[i]);
        ...
        { /* do fft */
          {
          vsip_fft_d *fft = vsip_ccfftop_create_d(np[i],1.0,VSIP_FFT_INV,0,0);
          printf("x = ");VU_cvprintm_d("8.6",x);
          vsip_ccfftop_d(fft,x,y);
        ...
        vsip_cvsub_d(y,cramp,chk); vsip_cvmag_d(chk,chk_r);
