...
static void cvouter_f(){
   printf("********\nTEST cvouter_f\n");
   {
     ...
     vsip_cblock_f  *block_b = vsip_cblockbind_f(data_b_r,data_b_i,4,0);
     vsip_cblock_f  *block_c = vsip_cblockcreate_f(100,VSIP_MEM_NONE);
     vsip_cblock_f  *ans_block = vsip_cblockbind_f(ans_r,ans_i,24,0);
     ...
     vsip_cmview_f  *chk = vsip_cmcreate_f(6,4,VSIP_ROW,0);
     ...
     VU_cmsub_f(c,ansm,chk); VU_cmmag_f(chk,chk_i); VU_mclip_f(chk_i,.0001,.0001,0,1,chk_i);
     if(VU_msumval_f(chk_i) > .5) 
         printf("error\n");
     else 
         printf("correct\n");
