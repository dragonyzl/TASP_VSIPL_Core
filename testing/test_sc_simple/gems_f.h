...
/* $Id: gems.t,v 1.4 2000/04/14 12:27:55 judd Exp $ */
static void gems_f(){
   printf("********\nTEST gems_f\n");
   {
   ...
   11.5,    4.6,    2.5 };                                     
     ...
     vsip_block_f *block_ans_n = vsip_blockbind_f(ans_n_data,15,0);
     vsip_block_f *block_ans_t = vsip_blockbind_f(ans_t_data,15,0);
     ...
     vsip_mview_f *chk_n = vsip_mcreate_f(3,5,VSIP_COL,0);
     vsip_mview_f *chk_t = vsip_mcreate_f(5,3,VSIP_ROW,0);
     ...
     VU_msub_f(cn,ans_n,chk_n); VU_mmag_f(chk_n,chk_n); VU_mclip_f(chk_n,.0001,.0001,0,1,chk_n);
     if(VU_msumval_f(chk_n) > .5)
         printf("error\n");
     else
         printf("correct\n");
     ...
     VU_msub_f(ct,ans_t,chk_t); VU_mmag_f(chk_t,chk_t); VU_mclip_f(chk_t,.0001,.0001,0,1,chk_t);
     if(VU_msumval_f(chk_t) > .5)
         printf("error\n");
     else
         printf("correct\n");
