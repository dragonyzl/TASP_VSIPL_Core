...
/* $Id: cvouter.t,v 1.5 2000/04/14 12:27:55 judd Exp $ */
static void cvouter_d(){
   printf("********\nTEST cvouter_d\n");
   {
     ...
     vsip_cblock_d  *block_b = vsip_cblockbind_d(data_b_r,data_b_i,4,0);
     vsip_cblock_d  *block_c = vsip_cblockcreate_d(100,VSIP_MEM_NONE);
     vsip_cblock_d  *ans_block = vsip_cblockbind_d(ans_r,ans_i,24,0);
     ...
     vsip_cmview_d  *chk = vsip_cmcreate_d(6,4,VSIP_ROW,0);
     ...
     VU_cmsub_d(c,ansm,chk); VU_cmmag_d(chk,chk_i); VU_mclip_d(chk_i,.0001,.0001,0,1,chk_i);
     if(VU_msumval_d(chk_i) > .5) 
         printf("error\n");
     else 
         printf("correct\n");
