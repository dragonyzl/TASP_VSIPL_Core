...
/* $Id: vrecip.t,v 1.3 2000/04/14 12:27:55 judd Exp $ */
static void vrecip_d(){
   printf("********\nTEST vrecip_d\n");
   {
     vsip_scalar_d data[] = {-1,  1, .1,  2, -4 };
     vsip_scalar_d ans[]  = { -1, 1, 10, .5,  -.25};
     vsip_block_d  *block = vsip_blockbind_d(data,5,0);
     vsip_block_d  *ans_block = vsip_blockbind_d(ans,5,0);
     vsip_vview_d  *a = vsip_vbind_d(block,0,1,5);
     vsip_vview_d  *ansv = vsip_vbind_d(ans_block,0,1,5);
     vsip_vview_d  *b = vsip_vcreate_d(15,0);
     vsip_vview_d  *chk = vsip_vcreate_d(5,0);
     vsip_vputlength_d(b,5);
     vsip_blockadmit_d(block,VSIP_TRUE);
     vsip_blockadmit_d(ans_block,VSIP_TRUE);
     vsip_vrecip_d(a,b);
     printf("vsip_vrecip_d(a,b)\n vector a\n");VU_vprintm_d("8.6",a);
     printf("vector b\n");VU_vprintm_d("8.6",b);
     ...
     vsip_vsub_d(a,ansv,chk); vsip_vmag_d(chk,chk); vsip_vclip_d(chk,.0001,.0001,0,1,chk);
     if(vsip_vsumval_d(chk) > .5) 
         printf(" vrecip_d in error for in place\n");
     else {
         printf("correct\n");
     }
