...
/* $Id: cvgather.t,v 1.3 2000/04/14 12:27:55 judd Exp $ */
static void cvgather_d(){
   printf("********\nTEST cvgather_d\n");
   {
        ...
        vsip_cvview_d *a = vsip_cvbind_d(
                  vsip_cblockbind_d(data1_r,data1_i,14,0),0,1,14);

        vsip_vview_vi *index = vsip_vbind_vi(/* matrix index is always interleaved */
                  vsip_blockbind_vi(data_index,5,0),0,1,5);

        vsip_cvview_d *ans = vsip_cvbind_d(
                  vsip_cblockbind_d(data_ans_r,data_ans_i,5,0),0,1,5);

        vsip_cvview_d *c = vsip_cvcreate_d(5,0);
