...
/* $Id: cvconj.t,v 1.2 2000/04/14 12:27:55 judd Exp $ */
static void cvconj_d(){
   printf("********\nTEST cvconj_d\n");
   {
     vsip_scalar_d data_r[] = {-1,  1, 0,  2, -2 };
     vsip_scalar_d data_i[] = {-.1,  .1, .0,  .2, -.2 };
     vsip_scalar_d ans_r[]  = { -1,  1, 0,  2, -2 };
     vsip_scalar_d ans_i[]  = { .1, -.1, 0, -.2,  .2};
     vsip_cblock_d  *block = vsip_cblockbind_d(data_r,data_i,5,0);
     vsip_cblock_d  *ans_block = vsip_cblockbind_d(ans_r,ans_i,5,0);
     vsip_cvview_d  *a = vsip_cvbind_d(block,0,1,5);
     vsip_cvview_d  *ansv = vsip_cvbind_d(ans_block,0,1,5);
     vsip_cvview_d  *b = vsip_cvcreate_d(15,0);
     vsip_cvview_d  *chk = vsip_cvcreate_d(5,0);
     ...
     vsip_cvsub_d(a,ansv,chk); vsip_cvmag_d(chk,chk_r); vsip_vclip_d(chk_r,.0001,.0001,0,1,chk_r);
     if(vsip_vsumval_d(chk_r) > .5) 
         printf(" vconj_d in error for in place\n");
     else {
         printf("correct\n");
     }
