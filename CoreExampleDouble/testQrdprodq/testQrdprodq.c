...
{  if(argc < 3){
   ...
   { /* test prodq for Q1 */
     ...
     vsip_mview_d *Q = vsip_msubview_d(I,0,0,M,M);
     vsip_mview_d *Qt = vsip_mtransview_d(Q);
     /* create a matrix to hold R */
     vsip_mview_d *A1 = vsip_mcreate_d(M,N,VSIP_ROW,VSIP_MEM_NONE);
     vsip_mview_d *R1 = vsip_msubview_d(A1,0,0,M,N);
     vsip_mview_d *R  = vsip_mcreate_d(M,N,VSIP_ROW,VSIP_MEM_NONE);
     ...
     /* do decomposition */
     vsip_qrd_d(qrd,A);
     printf("A decomposed = "); VU_mprintm_d("6.4",A);
     /* find Q */
     ...
     vsip_mcopy_d_d(Qt,I1);
     vsip_mprod_d(I1,Q,I2);
     printf("directly using mprod Q^t Q = \n"); VU_mprintm_d("6.4",I2);

     /* find Q^t Q using matprod on Rside */
     VU_mfillz_d(I1);
     vsip_mcopy_d_d(Qt,I1);
     vsip_qrdprodq_d(qrd,VSIP_MAT_NTRANS,VSIP_MAT_RSIDE,I1);
