...
#define F2 150
#define F3 50
#define Theta_o 30 /* beam number of narrow band tones */
...
#define kaiser 100 /* window parameter */
#define Nfilter 20 /* kernel length for noise filter */

void VU_mprintgram_d(vsip_mview_d*,char*);
...
{  int i,j; /* counters */
   ...
   vsip_vview_d *kernel = 
         vsip_vcreate_kaiser_d(Nfilter,kaiser,0);
   vsip_fir_d *fir = vsip_fir_create_d(kernel,
   ...
   vsip_mview_d *data = vsip_mcreate_d(Mp,Ns,VSIP_ROW,0),
                *rmview;
   vsip_vview_d *data_v, *gram_v;
   vsip_cvview_d *gram_data_v;
   vsip_cmview_d *gram_data = 
        vsip_cmcreate_d(Mp,Ns/2 + 1,VSIP_COL,0);
   vsip_mview_d  *gram = 
         vsip_mcreate_d(Mp,Ns/2 + 1,VSIP_ROW,0);
   vsip_mview_d  *Xim = 
         vsip_mcreate_d(Mp,Mp+1,VSIP_ROW,0);
   ...
   vsip_fftm_d *rcfftmop_obj = /* time fft */
           vsip_rcfftmop_create_d(Mp,Ns,1,VSIP_ROW,0,0);
   vsip_fftm_d *ccfftmip_obj =
           vsip_ccfftmip_create_d(Mp,Ns/2 +
               1,VSIP_FFT_FWD,1,VSIP_COL,0,0);
   vsip_vramp_d(0,1,m);
   vsip_vramp_d(0,M_PI/Mp,Xi); 
   vsip_vcos_d(Xi,Xi);
   vsip_vouter_d(alpha,m,Xi,Xim);
   { vsip_vview_d *gram_v = vsip_mrowview_d(gram,0);
     vsip_vputlength_d(gram_v,Mp*(Ns/2 + 1));
     vsip_vfill_d(0,gram_v);
   ...
   for(j=0; j<Nnoise; j++){
      ...
      printf("noise ="); VU_vprintm_d("6.4",noise[j]);
