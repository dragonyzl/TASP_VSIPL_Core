...
int main(){vsip_init((void*)0);
{
   ...
   { /* store data ; upper trianglar so answers are obvious */
     ...
     vsip_mput_f(A,1,0,0); vsip_mput_f(A,1,1,3); vsip_mput_f(A,1,2,0);vsip_mput_f(A,1,3,1);
     vsip_mput_f(A,2,0,0); vsip_mput_f(A,2,1,0); vsip_mput_f(A,2,2,2);vsip_mput_f(A,2,3,1);
   ...
   { /* solve using LUD */
      ...
      vsip_lusol_f(luAop,VSIP_MAT_TRANS,ATB);
      printf("solve A x = b\n");
      printf("x from LUD = "); VU_mprintm_f("6.4",ATB);
   ...
   { /* solve using qrdsolr */
     ...
     vsip_qrdsolr_f(qrd,VSIP_MAT_TRANS,1,B);
     printf("X from qrdsolr NOSAVEQ =");VU_mprintm_f("6.4",B);
   ...
   { /* solve using qrdsolr */
     vsip_qr_f *qrd;
     vsip_mview_f *B = vsip_mcreate_f(4,1,VSIP_COL,0);
     qrd = vsip_qrd_create_f(4,4,VSIP_QRD_SAVEQ);
     vsip_qrd_f(qrd,ATA);
     vsip_mput_f(B,0,0,1); vsip_mput_f(B,1,0,2); vsip_mput_f(B,2,0,2); vsip_mput_f(B,3,0,1);
     vsip_qrdsolr_f(qrd,VSIP_MAT_TRANS,1,B);
     printf("X from qrdsolr SAVEQ1 =");VU_mprintm_f("6.4",B);
     vsip_qrd_destroy_f(qrd);
    }
   vsip_mdestroy_f(A);
   vsip_mdestroy_f(ATA);
