...
void VU_cmfillz_f(vsip_cmview_f* Z)
{
     ...
     for(i=0; i<m; i++)
        for(j=0; j<n; j++)
          vsip_cmput_f(Z,i,j,vsip_cmplx_f(1.0,1.0));
     return;
}
...
{  if(argc < 3){
   ...
   { /* test prodq for Q1 */
     /* create a QRD object */
     vsip_cqr_f *qrd   = vsip_cqrd_create_f(M,N,VSIP_QRD_SAVEQ1);
     /* create a matrix to hold Q */
     vsip_cmview_f *I = VU_cI_f(M);
     vsip_cmview_f *Q = vsip_cmsubview_f(I,0,0,M,N);
     ...
     vsip_cmview_f *R1 = vsip_cmsubview_f(A1,0,0,N,N);
     vsip_cmview_f *R  = vsip_cmcreate_f(N,N,VSIP_ROW,VSIP_MEM_NONE);
     /* create a matrix to hold Q Q^h */
     vsip_cmview_f *I1 = vsip_cmcreate_f(M,M,VSIP_ROW,VSIP_MEM_NONE);
     vsip_cmview_f *I_1NN = vsip_cmsubview_f(I1,0,0,N,N);
     vsip_cmview_f *I_1MN = vsip_cmsubview_f(I1,0,0,M,N);
     vsip_cmview_f *I_1NM = vsip_cmsubview_f(I1,0,0,N,M);
     /* create a matrix to hold Q^h Q */
     vsip_cmview_f *I2 = vsip_cmcreate_f(M,M,VSIP_ROW,VSIP_MEM_NONE);
     vsip_cmview_f *I_2NN = vsip_cmsubview_f(I2,0,0,N,N);
     vsip_cmview_f *I_2MN = vsip_cmsubview_f(I2,0,0,M,N);
     vsip_cmview_f *I_2NM = vsip_cmsubview_f(I2,0,0,N,M);
     /* do decomposition */
     vsip_cqrd_f(qrd,A);
     /* find Q */
     ...
     vsip_cmherm_f(Q,I_1NM);
     vsip_cmprod_f(I_1NM,Q,I_2NN);
     printf("directly using mprod Q^h Q = "); VU_cmprintm_f("6.4",I_2NN);

     /* find Q^h Q using matprod on Rside */
     VU_cmfillz_f(I1);
     vsip_cmherm_f(Q,I_1NM);
     vsip_cqrdprodq_f(qrd,VSIP_MAT_NTRANS,VSIP_MAT_RSIDE,I_1NM);
     printf("using qrdprodq Q^h Q Rside = "); VU_cmprintm_f("6.4",I_1NN);

     /* find Q^h Q using matprod on Lside */
     VU_cmfillz_f(I2);
     vsip_cmcopy_f_f(Q,I_2MN);
     vsip_cqrdprodq_f(qrd,VSIP_MAT_HERM,VSIP_MAT_LSIDE,I2);
     printf("using qrdprodq Q^h Q Lside = "); VU_cmprintm_f("6.4",I_2NN);
    
     /* find Q Q^h using Mprod */
     VU_cmfillz_f(I1); VU_cmfillz_f(I2);
     vsip_cmherm_f(Q,I_1NM);
     vsip_cmprod_f(Q,I_1NM,I2);
     printf("directly using mprod Q Q^h = "); VU_cmprintm_f("6.4",I2);
      
     /* find Q Q^h using matprod on Rside */
     VU_cmfillz_f(I1);
     vsip_cmcopy_f_f(Q,I_1MN);
