...
**********************************************************************/
/* $Id: vsip_qrdprodq_d.c,v 1.1 1999/12/05 16:08:12 judd Exp $ */
     ...
     vsip_qr_d *qr)
{
     ...
     for(k= 0; k < H->row_length; k++){
         ...
         vsip_vcopy_d_d(h,v);
         vsip_vput_d(v,(vsip_index) 0, (vsip_scalar_d)1.0);
         vv = *(qr->v);
       ...
       vsip_qr_d *qr)
{
     ...
     for(k = qr->N -1; k >= 0; k--){
          ...
          v->offset = qr->M - h->length;
          vsip_vcopy_d_d(h,v);
          vsip_vput_d(v,(vsip_index)0,(vsip_scalar_d)1);
          vv = *(qr->v);
          v->length = C->col_length;
         ...
         const vsip_mview_d *A)
{
  ...
  vsip_qr_d    qrqr = *qr;
  int retval = 0;
  vsip_length q_row_length;
  if(qr->qopt == VSIP_QRD_NOSAVEQ){ 
         return 1;
  } else if(qr->qopt == VSIP_QRD_SAVEQ1){ 
         q_row_length = qr->N;
  } else if(qr->qopt == VSIP_QRD_SAVEQ) {
         q_row_length = qr->M;
  } else { 
         return 1;
  }
