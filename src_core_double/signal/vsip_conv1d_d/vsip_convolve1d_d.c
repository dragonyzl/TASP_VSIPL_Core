...
**********************************************************************/
/* $Id: vsip_convolve1d_d.c,v 1.1 1999/12/05 02:07:19 judd Exp judd $ */
#include<vsip.h>
       ...
       const vsip_vview_d *y)
{
  if(conv->method == 0){ /*kernel >= N/2 or kernel > 1000 */
     ...
     vsip_cvfill_d(vsip_cmplx_d(0,0),conv->x);
     /* copy input vector into work area */
     xt->length = xtn2; vsip_vcopy_d_d(xt,xr);
     xt->offset = xtn2; xt->length = x->length - xtn2; vsip_vcopy_d_d(xt,xi);
     ...
     if(conv->support == VSIP_SUPPORT_SAME){
        ...
        yt->length = xr->length;
        vsip_vcopy_d_d(xr,yt);
        yt->length = xi->length;
        yt->offset += (yt->stride * xr->length);
        vsip_vcopy_d_d(xi,yt);
     }
     ...
     { /* make sure we start with an initialized zero matrix */
          ...
          vsip_cvfill_d(vsip_cmplx_d(0,0),XVt);
     }
