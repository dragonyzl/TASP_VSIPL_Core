...
**********************************************************************/
/* $Id: vsip_conv1d_create_d.c,v 1.1 1999/12/05 02:07:17 judd Exp judd $ */
     ...
     vsip_alg_hint hint)
{
     ...
     { /* copy h to conv object */
        ...
        vsip_cvfill_d(vsip_cmplx_d(0,0),conv->H);
        switch(symm){
            case 0:{
               Hr->length = klength;
               vsip_vcopy_d_d(h,Hr);
     ...
     if((((float)N /(float) klength) > 2) && (klength < 1000) ){
         ...
         if((conv->fft = vsip_ccfftip_create_d(conv->Nsection,1,VSIP_FFT_FWD,0,0)) == NULL){
               vsip_conv1d_destroy_d(conv);
               return(vsip_conv1d_d*) NULL;
         }
         ...
         { vsip_length m,n;
           ...
           conv->NumSection = m;
           m = (m % 2) ? (m/2 + 1) : m/2; 
           conv->fftm = vsip_ccfftmip_create_d(m,conv->Nsection, 1, VSIP_FFT_FWD, VSIP_ROW,0,0);
         ...
         conv->fft = vsip_ccfftip_create_d(conv->Nsection,1,VSIP_FFT_FWD,0,0);
         if((conv->x == NULL) ||
            (conv->fft == NULL)){
                 vsip_conv1d_destroy_d(conv);
                 return (vsip_conv1d_d*)NULL;
