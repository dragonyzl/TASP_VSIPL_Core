...
**********************************************************************/
/* $Id: vsip_firflt_d.c,v 1.2 2000/02/15 21:26:55 judd Exp judd $ */
        ...
        const vsip_vview_d *yc)
{
    ...
    while(x->length < fir->M){
        vsip_vput_d(y,k++,vsip_vdot_d(h1,fir->s)+vsip_vdot_d(h2,x));
        x->length += fir->D;
        fir->s->length -= fir->D;
        fir->s->offset += fir->D;
        h1->length = fir->s->length;
    ...
    while(k < nout){ /* do the rest of the pieces */
       vsip_vput_d(y,k++,vsip_vdot_d(fir->h,x));
       x->offset += oinc;
    }
    ...
    if((fir->s->length > 0) && (fir->state == VSIP_STATE_SAVE)) 
          vsip_vcopy_d_d(x,fir->s);
    if(fir->state == VSIP_STATE_NO_SAVE){
         vsip_vfill_d(0,fir->s);
         fir->p = 0;
     }
    return k;
}
