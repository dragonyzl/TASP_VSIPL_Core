...
**********************************************************************/
/* $Id: vsip_ccfftop_d.c,v 1.1 1999/12/05 02:01:19 judd Exp judd $ */
#include<vsip.h>
...
void VI_dft_d(vsip_fft_d *fft) {
  ...
  vsip_scalar_d *apip = (vsip_scalar_d*) ((a->block->I->array) + cast * a->offset),
                ...
                *tbpi,
                *cpi = (vsip_scalar_d*) ((c->block->I->array) + cbst * c->offset);
  b->length = N;
  c->length = N;
  for(j=0; j<N; j++){
    apr = aprp; api = apip;
...
void VI_fft2_d(vsip_fft_d* fft)
{
   vsip_scalar_d *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i; 
...
void VI_ifft2_d(vsip_fft_d* fft)
{
   vsip_scalar_d *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i; 
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft3_d(vsip_fft_d *fft)
{
   vsip_scalar_d *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft4_d(vsip_fft_d *fft)
{
   vsip_scalar_d *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft5_d(vsip_fft_d *fft)
{
   vsip_scalar_d *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft7_d(vsip_fft_d* fft)
{
   vsip_scalar_d *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft8_d(vsip_fft_d* fft)
{
   vsip_scalar_d *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_fft_wt_d(vsip_fft_d *fft)
{
      vsip_cvview_d *a = fft->x;
      vsip_cvview_d *b = fft->wt;
      vsip_cvview_d *r = fft->x;
      vsip_stride cast = a->block->cstride;
      vsip_stride cbst = b->block->cstride;
...
void VI_p0pF_d(vsip_fft_d *fft)
{
  ...
  void (*fft_p)(vsip_fft_d*) = VI_p0pF_d;
  ...
  if(fft->d == -1){ /*forward section */
      switch( p0){ /* fft building block for p0 */
        case 2  : fft_p = VI_fft2_d; break;
        case 3  : fft_p = VI_fft3_d; break;
        case 4  : fft_p = VI_fft4_d; break;
        case 5  : fft_p = VI_fft5_d; break;
        case 7  : fft_p = VI_fft7_d; break;
        case 8  : fft_p = VI_fft8_d; break;
      }
      for(k=0; k<M-1; k++ ) { /* (M-1 segments) *( pF building blocks of length p0)*/
        ...
        for(j=1; j<Ns; j++){
          for(i = j; (int)i < N; i+= N1){
            ...
            fft->xp_i = fft->x_i + fft->xst * i;
            fft_p(fft);
            VI_fft_wt_d(fft); /*vsip_cvmul_d(fft->x,wt,fft->x);*/
          }
          fft->wt->stride += (seg * wtstr);
        }  
      seg *= p0 ;
      ...
      switch(case_pF){
        ...
        case 7  : fft_p = VI_fft7_d; break;
        case 8  : fft_p = VI_fft8_d; break;
        case 101: fft_p = VI_dft_d; break;
        case 103: fft_p = VI_p0pF_d; break;
      }
      for(j=0; j<Ns; j++){
        for(i = j; (int)i < N; i+= N1){
          fft->x->offset = xattr.offset + xattr.stride * i;
      ...
      switch( p0){ /* fft building block for p0 */
        case 2  : fft_p = VI_ifft2_d; break;
        case 3  : fft_p = VI_ifft3_d; break;
        case 4  : fft_p = VI_ifft4_d; break;
        case 5  : fft_p = VI_ifft5_d; break;
        case 7  : fft_p = VI_ifft7_d; break;
        case 8  : fft_p = VI_ifft8_d; break;
      }  
      for(k=0; k<M-1; k++ ) { /* (M-1 segments) *( pF building blocks of length p0)*/
        N1 = Ns;
        Ns = Ns/p0;
...
/* init fft to x */
void init_fft_d(vsip_cvview_d *x,vsip_fft_d* fft){
  fft->x = x;
  fft->stage =0;
  return;
}

void VI_ccfftip_d(const vsip_fft_d* fft, const vsip_cvview_d *x)
{
  init_fft_d((void*)x,(void*)fft);
  if(fft->dft == 1){
     VI_dft_d((void*)fft);
  }else{
     VI_p0pF_d((void*)fft);
     VI_sort_copy_d((void*)fft);
  }
...
/*========================================================*/
void VI_sort_copy_d(vsip_fft_d* fft){
