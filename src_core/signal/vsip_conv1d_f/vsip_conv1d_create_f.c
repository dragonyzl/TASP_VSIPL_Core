...
**********************************************************************/
/* $Id: vsip_conv1d_create_f.c,v 1.1 1999/12/02 18:45:19 judd Exp $ */
     ...
     vsip_alg_hint hint)
{
     ...
     { /* copy h to conv object */
        ...
        vsip_cvfill_f(vsip_cmplx_f(0,0),conv->H);
        switch(symm){
            case 0:{
               Hr->length = klength;
               vsip_vcopy_f_f(h,Hr);
     ...
     if((((float)N /(float) klength) > 2) && (klength < 1000) ){
         ...
         if((conv->fft = vsip_ccfftip_create_f(conv->Nsection,1,VSIP_FFT_FWD,0,0)) == NULL){
               vsip_conv1d_destroy_f(conv);
               return(vsip_conv1d_f*) NULL;
         }
         ...
         { int m = 0, n;
           ...
           m = (m % 2) ? (m/2 + 1) : m/2; 
           conv->fftm = vsip_ccfftmip_create_f(m,conv->Nsection, 1, VSIP_FFT_FWD, VSIP_ROW,0,0);
         ...
         conv->fft = vsip_ccfftip_create_f(conv->Nsection,1,VSIP_FFT_FWD,0,0);
         if((conv->x == NULL) ||
            (conv->fft == NULL)){
                 vsip_conv1d_destroy_f(conv);
                 return (vsip_conv1d_f*)NULL;
