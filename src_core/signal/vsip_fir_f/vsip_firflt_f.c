...
**********************************************************************/
/* $Id: vsip_firflt_f.c,v 1.2 2000/02/15 21:26:00 judd Exp judd $ */
        ...
        const vsip_vview_f *yc)
{
    ...
    while(x->length < fir->M){
        vsip_vput_f(y,k++,vsip_vdot_f(h1,fir->s)+vsip_vdot_f(h2,x));
        x->length += fir->D;
        fir->s->length -= fir->D;
        fir->s->offset += fir->D;
        h1->length = fir->s->length;
    ...
    while(k < nout){ /* do the rest of the pieces */
       vsip_vput_f(y,k++,vsip_vdot_f(fir->h,x));
       x->offset += oinc;
    }
    ...
    if((fir->s->length > 0) && (fir->state == VSIP_STATE_SAVE)) 
          vsip_vcopy_f_f(x,fir->s);
    if(fir->state == VSIP_STATE_NO_SAVE){
         vsip_vfill_f(0,fir->s);
         fir->p = 0;
     }
    return k;
}
